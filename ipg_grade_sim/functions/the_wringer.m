...
for q = 1:length(subtbl_array)
    ...
    subtbl_array{q}.P_AD_raw = subtbl.mass_eff.*subtbl.v.*(subtbl.a_modeled_w_drr-subtbl.a_estimate);
    ...
    subtbl_array{q}.P_AD_raw(~subtbl.decel_on)=0;
...
%         100*(trapz(subtbl_array{q}.P_AD_cadj)-trapz(subtbl_array{q}.PwrL_Brake))/trapz(subtbl_array{q}.PwrL_Brake))
    P_AD_inf{q} = [mean(subtbl_array{q}.P_AD_raw), mean(subtbl_array{q}.P_AD_rls), mean(subtbl_array{q}.P_AD_cadj)];
