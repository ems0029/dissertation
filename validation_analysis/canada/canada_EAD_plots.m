...
for q  = 1:length(files)
    ...
    tbl = add_features(tbl,'A2_canada.mat',[1400:30000],false);
    tbl.P_aero =(1-tbl.drag_reduction_ratio).*0.5*5.0*1.25.*tbl.v.^3;
    figure(1)
    hold on
    ...
    [meanExp,confint]=lm.predict( ...
        ...
        DRF_adj,fan_pwr],'Prediction','curve');
    ...
    [meanExp,confint]=lm.predict( ...
        [norm_EAD(tbl.E_AD,median(tbl.mass_eff),tbl.time-tbl.time(1)), ...
        0*DRF,fan_pwr],'Prediction','curve');
    expected(q) = plot(tbl.time,(recenterScale(meanExp))*-100);
    % confint plot
    plot(tbl.time,(recenterScale(confint))*-100,'LineStyle','--','Color',[0.5,0.5,0.5])
    ...
    mean(tbl.engine_power)/(mean(tbl.engine_power)-mean(tbl.P_AD_cadj)+mean(tbl.P_aero))
end


figure(1)
legend('75'' IVD with Cut-ins','75'' IVD ref #1','75'' IVD ref #2','75'' IVD ref #3')
xlabel('Time [s]')
...
legend(gca,[expected,conf],{'75'' IVD with Cut-ins','75'' IVD ref #1','75'' IVD ref #2','75'' IVD ref #3','Baseline NFC_{true}','Cut-in NFC_{true}'})
xlabel('Time [s]')
...
legend([expected,conf2],{'75'' IVD with Cut-ins','75'' IVD ref #1','75'' IVD ref #2','75'' IVD ref #3','Published NFC_{true}'})
xlabel('Time [s]')
ylabel('Impact of E_{AD} on NFC [%] ')
for q = 1:4
    figure(q)
    set(figure(q).Children,'FontSize',12)
    set(figure(q).Children(2),'xlim',[140,3000])
    set(figure(q).Children(2).Children,'linewidth',1.25)
    set(figure(q).Children(1),'Location','best','fontsize',10,'Orientation','Horizontal','NumColumns',2)
end
