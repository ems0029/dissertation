...
nfc_tbl_aug.delF_inf = ...
    kappa(eta)*(-(P_AD_ref+P_aero_ref.*DRR_ref) + (P_AD_plat + P_aero_plat.*DRR_plat));
nfc_tbl_aug.delFAD = ...
    kappa(eta)*((P_AD_plat)-(P_AD_ref));
nfc_tbl_aug.delF_fan = ...
    kappa(eta)*1000*(nfc_tbl_aug.mean_fan_power_est_T_plat-nfc_tbl_aug.mean_fan_power_est_T_ref);
nfc_tbl_aug.delFaero = ...
    kappa(eta)*((P_aero_plat.*DRR_plat)-(P_aero_ref.*DRR_ref));
end